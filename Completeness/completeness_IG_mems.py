...
output_file_name = "final_Completeness_mems_IG_DNN.txt" # mems rename
...
# # Define function to test sample with Integrated Gradients
...
#     ig = tfa.explain.integrated_gradients.integrated_gradients(model, sample, baseline, target_class_idx=predicted_class)
#     ig = tf.reduce_sum(tf.abs(ig), axis=0).numpy()

#     feature_importance = pd.DataFrame({
#         'feature': X_test.columns,
#         'feature_value': sample.numpy().flatten(),
#         'ig_values': ig
#     })

#     feature_importance.sort_values(by=['ig_values'], ascending=False, inplace=True)

#     ig_values = feature_importance['ig_values'].tolist()
#     feature_val = feature_importance['feature_value'].tolist()
#     feature_name = feature_importance['feature'].tolist()

#     return (label[predicted_class], ig_values, feature_val, feature_name)
