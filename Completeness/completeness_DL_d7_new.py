...
# Reduce dataset to 4% of each class
# Calculate number of samples per class
num_samples_per_class = int(len(data) * 0.04 / len(label_map))

# Select subset of samples for each class
subset_data = pd.DataFrame()
for label_value, label_name in label_map.items():
    subset = data[data['label'] == label_name].sample(n=num_samples_per_class, random_state=42)
    subset_data = pd.concat([subset_data, subset], axis=0)
...
output_file_name = "Completeness_d7_DL_with_4%.txt"
...
# d7
benign_samples = subset_data[subset_data['label'] == 'benign']
print("Count of benign samples:", len(benign_samples))
